...
namespace Org.Apache.REEF.Fluid.Network
{
    public class MessageService : IObserver<IMessageInstance<object>>
    {
            ...
            [Parameter(typeof(MessageServiceOptions.MessageHandler))] IObserver<NsMessage<byte[]>> handler)
        {
            ...
            this.Receiver = this; 
            this.serializer = new ProtocolSerializer(this.GetType().Assembly, "Org.Apache.REEF.Fluid.Message");
        ...
        {
            this.connection.Write(serializer.Write(message, Interlocked.Increment(ref sequence)));
        }

        public void OnNext(IMessageInstance<object> instance)
        {
            Logr.Log(Level.Info, "OnNext(object): " + instance.Message.ToString());          
        }
